	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < NUM_KEYS; i++) {
			...
			if (!isBlackKey(pitch)) {
				...
				g.setColor((i == hoveredKeyIndex && isMousePressed) || key[i] == 1 ? Color.RED : whiteKeyColor);
...
//				}
		...
		for (int i = 0; i < NUM_KEYS; i++) {
			...
			if (isBlackKey(pitch)) {
				...
				g.setColor((i == hoveredKeyIndex && isMousePressed) || key[i] == 1 ? Color.RED : blackKeyColor);
